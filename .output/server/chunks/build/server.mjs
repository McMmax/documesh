import process from 'node:process';globalThis._importMeta_=globalThis._importMeta_||{url:"file:///_entry.js",env:process.env};import { defineComponent, shallowRef, getCurrentInstance, provide, cloneVNode, h, createElementBlock, hasInjectionContext, inject, defineAsyncComponent, useSlots, Fragment, computed, useSSRContext, createApp, reactive, toRaw, withCtx, createVNode, onErrorCaptured, onServerPrefetch, unref, resolveDynamicComponent, shallowReactive, effectScope, ref, watch, mergeProps, getCurrentScope, toRef, toValue, onScopeDispose, isReadonly, isRef, isShallow, isReactive } from 'vue';
import { w as withQuery, l as parseURL, e as encodePath, m as decodePath, n as klona, h as createError$1, o as hasProtocol, q as isScriptProtocol, r as joinURL, v as defuFn, x as sanitizeStatusCode, y as getContext, $ as $fetch$1, z as baseURL, A as defu, B as executeAsync } from '../_/nitro.mjs';
...
import { convertToAccessorMap, makeQueryBuilder, reducers as reducers$2, reducerSchema, schema, table, t, procedures, DbConnectionImpl, DbConnectionBuilder as DbConnectionBuilder$1, SubscriptionBuilderImpl } from 'spacetimedb';
...
const _routes = [
  {
    ...
    component: () => import('./_id_-Bawvu206.mjs')
  },
  {
    name: "dashboard",
    path: "/dashboard",
    meta: __nuxt_page_meta$1 || {},
    component: () => import('./dashboard-CJch8MmR.mjs')
  },
  {
    name: "index",
    path: "/",
    meta: __nuxt_page_meta || {},
    component: () => import('./index-Dn1YvUiR.mjs')
  }
];
...
const revive_payload_server_MVtmlZaQpj6ApFmshWfUWl5PehCebzaBf2NuRMiIbms = /* @__PURE__ */ defineNuxtPlugin({
  ...
  setup() {
    for (const [reducer, fn] of reducers$1) {
...
const LazyIcon = defineAsyncComponent(() => import('./index-BZauEo50.mjs').then((r) => r["default"] || r.default || r));
...
const AddCommentReplyReducer = {
  id: t.string(),
  commentId: t.string(),
  docId: t.string(),
  content: t.string()
};
const ApplyYjsUpdateReducer = {
  docId: t.string(),
  updateBytes: t.byteArray()
};
const DeleteCommentReducer = {
  id: t.string()
};
const DocumentCreateReducer = {
  id: t.string(),
  title: t.string()
};
const DocumentDeleteReducer = {
  id: t.string()
};
const DocumentSetLockedReducer = {
  id: t.string(),
  locked: t.bool()
};
const DocumentSetPinnedReducer = {
  id: t.string(),
  pinned: t.bool()
};
const DocumentSetPublicReducer = {
  id: t.string(),
  isPublic: t.bool()
};
const DocumentUpdateTitleReducer = {
  id: t.string(),
  title: t.string()
};
const ResolveCommentReducer = {
  id: t.string(),
  resolved: t.bool()
};
const SaveVersionReducer = {
  docId: t.string(),
  label: t.string(),
  snapshotBytes: t.byteArray()
};
const UpdateAwarenessReducer = {
  docId: t.string(),
  stateJson: t.string()
};
const UpsertSessionReducer = {
  displayName: t.string(),
  color: t.string()
};
const AwarenessStateRow = t.row({
  identity: t.identity().primaryKey(),
  docId: t.string().name("doc_id"),
  stateJson: t.string().name("state_json"),
  updatedAt: t.timestamp().name("updated_at")
});
const CommentRow = t.row({
  id: t.string().primaryKey(),
  docId: t.string().name("doc_id"),
  authorIdentity: t.identity().name("author_identity"),
  authorName: t.string().name("author_name"),
  authorColor: t.string().name("author_color"),
  content: t.string(),
  resolved: t.bool(),
  createdAt: t.timestamp().name("created_at"),
  updatedAt: t.timestamp().name("updated_at")
});
const CommentReplyRow = t.row({
  id: t.string().primaryKey(),
  commentId: t.string().name("comment_id"),
  docId: t.string().name("doc_id"),
  authorIdentity: t.identity().name("author_identity"),
  authorName: t.string().name("author_name"),
  content: t.string(),
...
const DocumentRow = t.row({
  id: t.string().primaryKey(),
  title: t.string(),
  ownerId: t.identity().name("owner_id"),
  isPinned: t.bool().name("is_pinned"),
  isLocked: t.bool().name("is_locked"),
  isPublic: t.bool().name("is_public"),
  createdAt: t.timestamp().name("created_at"),
  updatedAt: t.timestamp().name("updated_at")
});
const DocumentVersionRow = t.row({
  id: t.u64().primaryKey(),
  docId: t.string().name("doc_id"),
  label: t.string(),
  snapshotBytes: t.byteArray().name("snapshot_bytes"),
  createdByName: t.string().name("created_by_name"),
  createdAt: t.timestamp().name("created_at")
});
const UserSessionRow = t.row({
  identity: t.identity().primaryKey(),
  displayName: t.string().name("display_name"),
  color: t.string(),
  updatedAt: t.timestamp().name("updated_at")
});
const YjsUpdateRow = t.row({
  id: t.u64().primaryKey(),
  docId: t.string().name("doc_id"),
  updateBytes: t.byteArray().name("update_bytes"),
  clientId: t.u64().name("client_id"),
  createdAt: t.timestamp().name("created_at")
});
const tablesSchema = schema({
  awareness_state: table({
    name: "awareness_state",
    indexes: [
      { accessor: "identity", name: "awareness_state_identity_idx_btree", algorithm: "btree", columns: [
        "identity"
      ] }
    ],
    constraints: [
      { name: "awareness_state_identity_key", constraint: "unique", columns: ["identity"] }
    ]
  }, AwarenessStateRow),
  comment: table({
    name: "comment",
  ...
  comment_reply: table({
    name: "comment_reply",
    indexes: [
      { accessor: "id", name: "comment_reply_id_idx_btree", algorithm: "btree", columns: [
        "id"
      ] }
    ],
    constraints: [
      { name: "comment_reply_id_key", constraint: "unique", columns: ["id"] }
    ]
  }, CommentReplyRow),
  document: table({
    name: "document",
  ...
  document_version: table({
    ...
    indexes: [
      { accessor: "id", name: "document_version_id_idx_btree", algorithm: "btree", columns: [
        "id"
      ] }
    ],
    constraints: [
  ...
  user_session: table({
    name: "user_session",
    indexes: [
      { accessor: "identity", name: "user_session_identity_idx_btree", algorithm: "btree", columns: [
        "identity"
      ] }
    ],
    constraints: [
  ...
  yjs_update: table({
    name: "yjs_update",
    indexes: [
      { accessor: "id", name: "yjs_update_id_idx_btree", algorithm: "btree", columns: [
        "id"
      ] }
    ],
    constraints: [
      { name: "yjs_update_id_key", constraint: "unique", columns: ["id"] }
    ]
  }, YjsUpdateRow)
});
const reducersSchema = reducers$2(
  reducerSchema("add_comment", AddCommentReducer),
  reducerSchema("add_comment_reply", AddCommentReplyReducer),
  reducerSchema("apply_yjs_update", ApplyYjsUpdateReducer),
  reducerSchema("delete_comment", DeleteCommentReducer),
  reducerSchema("document_create", DocumentCreateReducer),
  reducerSchema("document_delete", DocumentDeleteReducer),
  reducerSchema("document_set_locked", DocumentSetLockedReducer),
  reducerSchema("document_set_pinned", DocumentSetPinnedReducer),
  reducerSchema("document_set_public", DocumentSetPublicReducer),
  reducerSchema("document_update_title", DocumentUpdateTitleReducer),
  reducerSchema("resolve_comment", ResolveCommentReducer),
  reducerSchema("save_version", SaveVersionReducer),
  reducerSchema("update_awareness", UpdateAwarenessReducer),
  reducerSchema("upsert_session", UpsertSessionReducer)
);
...
const reducers = convertToAccessorMap(reducersSchema.reducersType.reducers);
...
function loadAuth() {
  return { isGuest: true, username: null, color: COLORS[4], choiceMade: false };
}
const _auth = shallowRef(loadAuth());
computed(() => !_auth.value.choiceMade);
const _sfc_main$2 = /* @__PURE__ */ defineComponent({
  __name: "app",
  __ssrInlineRender: true,
  setup(__props) {
    const { public: { spacetimedbUri, spacetimedbModule } } = /* @__PURE__ */ useRuntimeConfig();
    const initName = "Anonymous";
    const initColor = "#14b8a6";
    DbConnection.builder().withUri(spacetimedbUri).withDatabaseName(spacetimedbModule).withToken(void 0).onConnect((conn, _identity, token) => {
      try {
        conn.reducers.upsertSession({ displayName: initName, color: initColor });
      } catch {
      }
    });
    return (_ctx, _push, _parent, _attrs) => {
      ...
      _push(ssrRenderComponent(_component_ClientOnly, _attrs, {
        fallback: withCtx((_, _push2, _parent2, _scopeId) => {
          if (_push2) {
            _push2(`<div class="flex h-screen items-center justify-center bg-white dark:bg-neutral-950"${_scopeId}><div class="size-6 animate-spin rounded-full border-2 border-mint-500 border-t-transparent"${_scopeId}></div></div>`);
          } else {
            return [
              createVNode("div", { class: "flex h-screen items-center justify-center bg-white dark:bg-neutral-950" }, [
                createVNode("div", { class: "size-6 animate-spin rounded-full border-2 border-mint-500 border-t-transparent" })
              ])
            ];
          }
        })
...
const _sfc_main$1 = {
  ...
  setup(__props) {
    ...
    const stack = void 0;
    const _Error404 = defineAsyncComponent(() => import('./error-404-CKyxZPEv.mjs'));
    const _Error = defineAsyncComponent(() => import('./error-500-B_TahtG9.mjs'));
...
export { __nuxt_component_1 as _, useRoute as a, useRouter as b, resolveRouteObject as c, useNuxtApp as d, entry_default as default, encodeRoutePath as e, useRuntimeConfig as f, nuxtLinkDefaults as g, useAppConfig as h, asyncDataDefaults as i, createError as j, navigateTo as n, reducers as r, tables as t, useHead as u };
